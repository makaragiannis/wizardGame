...
public class GameController implements Initializable {
	...
	@FXML
	private Button orderCardsButton, nextActionButton, showPlayedCardsButton;
...
//		System.out.println("Current State: " + actionIndex);
		...
		switch(actionIndex) {
			...
			case 2: {
				...
				actionIndex = 3;
				if (gameplay.getCurrentRound() != gameplay.getMaxRound()) setTrumpCard();
...
//				gameplay.resetTable();
	...
	void guessTricks(Player player) {
		...
		if (player.isUser() == true) {
			...
			if (trickNumber != null) gameplay.setTricksGuessed(player, trickNumber);
		}
		else {
	...
	void playCard(Player player) {
		...
		if (player.isUser() && !userPlayedCard) return;
